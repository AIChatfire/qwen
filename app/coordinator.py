...
惰性轮询（只在调用方 GET 时回查）做不到两件事，协调器补上：
  1. **超时看门狗**：没人查的任务会永远停在 `running`；协调器把超过 `TASK_TIMEOUT`
     的任务置 `expired`；
  2. **产物就绪度**：完成后尽快把 `succeeded` 落库，调用方首次 GET 即可见。

⚠️ 多 worker 部署要另做选主/租约去重（本服务默认 `WORKERS=1`，与 jimeng 同口径）；
⚠️ 它**只回查自己的任务记录**（账号在记录里），不引入任何新的上游写操作。
"""
